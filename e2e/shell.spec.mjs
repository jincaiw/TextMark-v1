...
describe("TextMark desktop shell", () => {
  ...
  it("supports native preview interactions, search, inspector and source-aware tables", async () => {
    ...
    for (const mode of modes) {
      if ((await mode.getText()).trim() === "开头为") { await mode.click(); break; }
    }
...
describe("TextMark toolbar click matrix", () => {
  ...
  it("open-actions dropdown lists editors and stays interactive", async () => {
    ...
    await expect(await $("//details[contains(@class,'open-with')]//div[contains(@class,'menu-popover')]//button[contains(normalize-space(),'系统默认编辑器')]")).toBeDisplayed();
  ...
  it("more menu lists every overflow action", async () => {
    ...
    for (const label of ["打开文件…", "打开文件夹…", "存储", "存储为…", "拷贝 Markdown 源文本", "打印…", "导出 HTML…", "导出 PDF", "导出 PNG…", "自定义工具栏…", "偏好设置…"]) {
