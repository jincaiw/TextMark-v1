...
export function renamedDestinationInDirectory(event: DiskChangeEvent, originalPath: string): string | null {
  ...
  for (let index = event.paths.length - 1; index >= 0; index -= 1) {
    ...
    if (!samePath(candidate, originalPath)
      && directory(candidate) === originalDirectory
      && isSupportedDocument(candidate)) return candidate;
  }
  return null;
}
...
export function resolveChangedDocumentPath(originalPath: string, originalExists: boolean, event: DiskChangeEvent, pairedDestination: string | null = null): string | null {
