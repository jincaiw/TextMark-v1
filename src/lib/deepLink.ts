/**
 * Accept only TextMark's documented file form: textmark://file/<absolute-path>.
...
export function parseTextmarkFileUrl(raw: string): string | null {
  ...
  if (!absolute || !/\.(?:md|markdown|mdown|mkd|mkdn|mdwn|mdtxt|mdtext|rmd|txt)$/i.test(path)) return null
  return path
}
