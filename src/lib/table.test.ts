...
describe('editMarkdownTable', () => {
  ...
  it('adds a row before the selected data row', () =>
    expect(editMarkdownTable(table, 0, 1, 0, { edit: 'addRowBefore' }).split('\n')[2]).toBe('|  |  |'))
  it('duplicates the selected data row', () =>
    expect(editMarkdownTable(table, 0, 1, 0, { edit: 'duplicateRow' }).match(/\| 1 \| 2 \|/g)).toHaveLength(2))
  ...
  it('adds a column before the selected column', () =>
    expect(editMarkdownTable(table, 0, 1, 0, { edit: 'addColumnBefore' }).split('\n')[0]).toBe('|  | A | B |'))
  it('duplicates a column and its alignment marker', () => {
    const source = '| A | B |\n| :--- | ---: |\n| 1 | 2 |'
    const next = editMarkdownTable(source, 0, 1, 1, { edit: 'duplicateColumn' })
    expect(next.split('\n')[1]).toBe('| :--- | ---: | ---: |')
  })
  it('will not delete the last remaining column', () => {
    const source = '| A |\n| --- |\n| 1 |'
    expect(editMarkdownTable(source, 0, 1, 0, { edit: 'deleteColumn' })).toBe(source)
  })
  it('edits the selected table when several exist', () => {
    const source = `${table}\n\n${table}`
    const next = editMarkdownTable(source, 1, 1, 1, { edit: 'setCell', value: 'second' })
    expect(next.indexOf('second')).toBeGreaterThan(next.indexOf(table))
  })
  it('leaves the source unchanged for a missing table', () =>
    expect(editMarkdownTable(table, 10, 1, 1, { edit: 'setCell', value: 'x' })).toBe(table))
  it('does not count the rendered frontmatter table in source-table coordinates', () => {
    const root = document.createElement('article')
    root.innerHTML =
      '<section class="md-frontmatter"><table><tbody><tr><td>meta</td></tr></tbody></table></section><table><tbody><tr><td>source</td></tr></tbody></table>'
  ...
  it('keeps empty editable headers accessible without touching metadata headers', () => {
    const root = document.createElement('article')
    root.innerHTML =
      '<section class="md-frontmatter"><table><thead><tr><th></th></tr></thead></table></section><table><thead><tr><th></th><th>Name</th></tr></thead></table>'
  ...
  it('delegates stable source coordinates without counting the frontmatter table', () => {
    const root = document.createElement('article')
    root.innerHTML =
      '<section class="md-frontmatter"><table><tr><td>meta</td></tr></table></section><table><thead><tr><th>A</th></tr></thead><tbody><tr><td>one</td></tr></tbody></table>'
    synchronizeTableSourceCoordinates(root, [
      {
        index: 0,
        startLine: 5,
        endLine: 7,
        ...
        columns: 1,
        cells: [
          { row: 0, column: 0, markdown: 'A' },
          { row: 1, column: 0, markdown: 'one' },
        ],
      },
    ])
