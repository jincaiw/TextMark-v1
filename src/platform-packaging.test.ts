...
describe('Linux package file mappings', () => {
  it.each(['deb', 'rpm'] as const)('maps %s package destinations to repository sources', (kind) => {
    expect(linuxConfig.bundle.linux[kind].files).toEqual({
      '/usr/share/mime/packages/app.textmark.xml': '../platform/linux/app.textmark.xml',
      '/usr/share/thumbnailers/textmark.thumbnailer': '../platform/linux/textmark.thumbnailer',
    })
  })

  it('maps the KDE thumbnail plugin destination to its built library', () => {
    expect(kdeConfig.bundle.linux.rpm.files).toEqual({
      '/usr/lib/textmark/platform/kde/textmarkthumbnail.so': '../platform/linux/build/package/textmarkthumbnail.so',
    })
  })
})
...
describe('native desktop package integration', () => {
  it('embeds the macOS Quick Look extension at the app bundle destination', () => {
    expect(macosConfig.bundle.macOS.files).toEqual({
      'PlugIns/TextMarkQuickLook.appex': '../platform/macos/build/TextMarkQuickLook.appex',
    })
  })
  ...
  it("links the Windows preview handler to Tauri's WiX install directory", () => {
    ...
    expect(windowsWixFragment).toContain('Value="[INSTALLDIR]TextMarkPreview\\TextMarkPreviewHandler.dll"')
  ...
  it('preserves spaced Windows installer and preview paths as single arguments', () => {
    expect(windowsInstallerTest).toContain('[System.Diagnostics.ProcessStartInfo]::new()')
    expect(windowsInstallerTest).toContain('$startInfo.ArgumentList.Add($argument)')
    expect(windowsInstallerTest).not.toContain('Start-Process -FilePath $FilePath -ArgumentList $Arguments')
  })

  it('waits for the detached NSIS uninstaller to finish registry cleanup', () => {
    expect(windowsInstallerTest).toContain('function Wait-InstallationRemoved')
    expect(windowsInstallerTest).toContain('Start-Sleep -Milliseconds 250')
    expect(windowsInstallerTest).toContain('TextMark installation state was not removed within $TimeoutSeconds seconds')
    expect(windowsInstallerTest).toContain('Test-Path $installedPreviewDll')
  ...
  it("keeps the macOS CLI smoke thumbnail's PNG extension intact", () => {
    expect(macosPackageTest).toContain('thumbnail="$cli_temp_dir/thumbnail.png"')
