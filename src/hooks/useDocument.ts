...
export function useDocument(locale: Locale, options: UseDocumentOptions = {}) {
    ...
    void listen<DiskChangeEvent>('disk-change', (event) => {
      ...
      if (workspacePath) {
        ...
        workspaceTimer = window.setTimeout(() => {
          void scanFolder(workspacePath).then(setFiles).catch(showError)
        }, 180)
      }
    }).then((dispose) => {
      if (disposed) dispose()
  ...
  const openPath = useCallback(
    async (path: string, newTab = false) => {
      if (!isTauri()) return
      setBusy(true)
      try {
  ...
  const openFolder = useCallback(async () => {
    if (!isTauri()) {
      setNotice(messages[locale].folderDesktopOnly)
      return
    }
    setBusy(true)
    try {
    ...
    async (force = false, automatic = false) => {
      ...
      try {
        ...
        if (errorCode(error) === 'save_conflict') {
          try {
            setExternalChange({ kind: 'modified', document: await readDocument(active.path) })
          } catch (readError) {
  ...
  const openRelative = useCallback(
    async (relativePath: string, scrollTop = 0) => {
