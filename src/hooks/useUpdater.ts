...
export type UpdateStatus = { state: "idle" | "checking" | "current" | "available" | "downloading" | "error"; version?: string; progress?: number };

export function useUpdater(channel: "stable" | "beta") {
  const [status, setStatus] = useState<UpdateStatus>({ state: "idle" });
  const checkNow = useCallback(async () => {
    if (!isTauri()) { setStatus({ state: "current" }); return; }
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => { void checkNow(); }, 5000);
