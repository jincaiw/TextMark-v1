  ...
  return <aside className="inspector-panel" aria-label={t(locale, "documentInfo")}>
    <header>
      <div className="inspector-segmented" role="tablist">
        <button role="tab" aria-selected={tab === "document"} className={tab === "document" ? "active" : ""} onClick={() => setTab("document")}><FileText />{t(locale, "document")}</button>
        <button role="tab" aria-selected={tab === "properties"} className={tab === "properties" ? "active" : ""} onClick={() => setTab("properties")}><Info />{t(locale, "properties")}</button>
      </div>
    ...
    {tab === "document" ? (
      ...
      <section className="inspector-tab">
        {frontmatter.length
          ? <dl className="frontmatter-list">{frontmatter.map((entry, index) => <div key={`${entry.key}-${index}`}><dt>{entry.key}</dt><dd>{entry.value}</dd></div>)}</dl>
