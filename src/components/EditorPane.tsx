...
const wrappers: Partial<Record<FormatCommand, [string, string]>> = {
  bold: ["**", "**"], italic: ["*", "*"], strikethrough: ["~~", "~~"], code: ["`", "`"], link: ["[", "](https://)"],
};
...
export const EditorPane = forwardRef<EditorPaneHandle, EditorPaneProps>(function EditorPane(props, forwardedRef) {
  ...
  useImperativeHandle(forwardedRef, () => ({
    ...
    format: (command) => { if (viewRef.current) applyFormat(viewRef.current, command); },
  }), []);
  ...
  return (
    <section className={`editor-pane content-${props.contentWidth}`} aria-label="Markdown editor" style={{ "--editor-zoom": props.zoom / 100 } as React.CSSProperties}>
      <div className="editor-page">
        <CodeMirror
          ...
          basicSetup={{ lineNumbers: false, foldGutter: false, highlightActiveLine: false, highlightActiveLineGutter: false, bracketMatching: true, closeBrackets: true }}
