...
export function SettingsDialog(props: SettingsDialogProps) {
  ...
  const updateText = props.updateStatus.state === "checking" ? t(props.locale, "checking")
    : props.updateStatus.state === "current" ? t(props.locale, "upToDate")
      : props.updateStatus.state === "available" ? t(props.locale, "updateAvailable", { version: props.updateStatus.version ?? "" })
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={props.onClose} ref={backdropRef}>
      <section className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <label><span>{t(props.locale, "updateChannel")}</span><select value={props.updateChannel} onChange={(event) => props.onUpdateChannelChange(event.target.value as AppSettings["updateChannel"])}><option value="stable">{t(props.locale, "stable")}</option><option value="beta">{t(props.locale, "beta")}</option></select></label>
        <div className="settings-update"><span>{updateText}</span>{props.updateStatus.state === "available" ? <button onClick={props.onInstallUpdate}>{t(props.locale, "installUpdate")}</button> : <button disabled={props.updateStatus.state === "checking" || props.updateStatus.state === "downloading"} onClick={props.onCheckUpdate}>{t(props.locale, "checkForUpdates")}</button>}</div>
        <label><span>{t(props.locale, "crashReports")}</span><span className="setting-toggle"><input type="checkbox" checked={props.crashReports} disabled={!props.crashReportsAvailable} onChange={(event) => props.onCrashReportsChange(event.target.checked)} aria-label={t(props.locale, "crashReports")} />{!props.crashReportsAvailable ? <small>{t(props.locale, "unavailable")}</small> : null}</span></label>
