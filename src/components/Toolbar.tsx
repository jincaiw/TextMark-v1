...
import type { ExternalApplication, Locale, SidebarMode, ToolbarItem, ViewMode } from "../types";

interface ToolbarProps {
  fileName: string; dirty: boolean; busy: boolean; viewMode: ViewMode; sidebarVisible: boolean; sidebarMode: SidebarMode;
  inspectorVisible: boolean; zoom: number; searchQuery: string; locale: Locale; items: ToolbarItem[];
  ...
  onOpen: () => void; onOpenFolder: () => void; onSave: () => void; onSaveAs: () => void; onShare: () => void; onPrint: () => void;
...
export function Toolbar(props: ToolbarProps) {
  ...
  const renderItem = (item: ToolbarItem, index: number) => {
    ...
    if (item === "sidebar") return <div key={key} className="sidebar-control">
      <button className={props.sidebarVisible ? "selected" : ""} title={tx("toggleSidebar")} aria-label={tx("toggleSidebar")} onClick={props.onToggleSidebar}><PanelLeft /></button>
    ...
    if (item === "openWith") return <details key={key} className="toolbar-group open-with">
      <summary title={tx("openWith")}><AppWindow /><ChevronDown /></summary>
      <div className="menu-popover"><b>{tx("openWith")}</b>{props.applications.filter((application) => application.kind !== "llm" && application.available).map((application) => <button key={application.id} onClick={() => props.onOpenWith(application.kind === "system" ? undefined : application.name)}>{application.kind === "system" ? tx("systemDefault") : application.name}</button>)}
    ...
    const actions: Partial<Record<ToolbarItem, { title: Parameters<typeof t>[1]; icon: React.ReactNode; active?: boolean; action: () => void }>> = {
      ...
      copy: { title: "copyItem", icon: <Clipboard />, action: props.onShare },
      export: { title: "exportItem", icon: <FileDown />, action: props.onExportHtml },
    };
    const action = actions[item];
    return action ? <button key={key} className={`toolbar-item-button ${action.active ? "selected" : ""} ${item === "edit" && action.active ? "edit-active" : ""}`} title={tx(action.title)} aria-label={tx(action.title)} onClick={action.action}>{action.icon}</button> : null;
  };
  ...
  return <header className="native-toolbar" data-tauri-drag-region onClick={(event) => {
    ...
    </div><div className="history-buttons"><button disabled={!props.canGoBack} aria-label="Back" onClick={props.onBack}><ChevronLeft /></button><button disabled={!props.canGoForward} aria-label="Forward" onClick={props.onForward}><ChevronRight /></button></div></div>
    ...
    <div className="native-actions">{props.items.map(renderItem)}
      <details className="more-menu"><summary title={tx("more")}><MoreHorizontal /></summary><div className="menu-popover align-right">
        ...
        <button onClick={props.onShare}><Clipboard />{tx("copySource")}</button><button onClick={props.onPrint}><Printer />{tx("print")}</button><button onClick={props.onExportHtml}><FileDown />{tx("exportHtml")}</button><button onClick={props.onExportPng}><FileDown />{tx("exportPng")}</button><hr />
