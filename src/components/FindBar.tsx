...
export function FindBar(props: FindBarProps) {
  const modeToggle = (value: SearchMode, label: Parameters<typeof t>[1]) => (
    <button role="button" aria-pressed={props.mode === value} className={`find-mode ${props.mode === value ? "selected" : ""}`} onClick={() => props.onModeChange(value)}>{t(props.locale, label)}</button>
  );
  return <div className="find-bar" role="search">
    ...
    <span className="find-count">{props.query ? (props.count ? t(props.locale, "matchCount", { current: props.current + 1, count: props.count }) : t(props.locale, "notFound")) : ""}</span>
    <button className={`find-icon ${props.matchCase ? "selected" : ""}`} title={t(props.locale, "matchCase")} aria-pressed={props.matchCase} onClick={() => props.onMatchCaseChange(!props.matchCase)}><CaseSensitive /></button>
    <button className="find-icon" title={t(props.locale, "previousMatch")} disabled={!props.count} onClick={props.onPrevious}><ChevronUp /></button>
    <button className="find-icon" title={t(props.locale, "nextMatch")} disabled={!props.count} onClick={props.onNext}><ChevronDown /></button>
    <button className="find-done" onClick={props.onClose}>{t(props.locale, "done")}</button>
  </div>;
}
