...
export function ConflictDialog({
  ...
  change: ExternalDocumentChange | null
  locale: Locale
  ...
  return (
    <div className="dialog-backdrop">
      ...
      >
        <h2 id="conflict-title">{t(locale, title)}</h2>
        <p id="conflict-body">{t(locale, body)}</p>
        <div>
          ...
          <button className="destructive" onClick={() => onResolve('overwrite')}>
            {t(locale, change.kind === 'modified' ? 'overwrite' : change.kind === 'renamed' ? 'recreateOriginal' : 'recreate')}
          </button>
        </div>
      </section>
    </div>
