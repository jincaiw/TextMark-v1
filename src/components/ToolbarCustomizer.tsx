...
const itemIcon = (item: ToolbarItem): React.ReactNode => {
  switch (item) {
    ...
    case 'exportPdf':
      return <FileDown />
    case 'flexibleSpace':
      return <span className="tc-glyph tc-glyph-flex" aria-hidden="true" />
    case 'space':
      return <span className="tc-glyph tc-glyph-space" aria-hidden="true" />
  }
}
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose} ref={backdropRef}>
      ...
      >
        <header>
          <div>
            <h2 id="toolbar-title">{t(locale, 'toolbarTitle')}</h2>
            <p>{t(locale, 'toolbarHint')}</p>
          </div>
        ...
        <footer>
          ...
          <label className="tc-display">
            <span>{t(locale, 'toolbarDisplay')}</span>
