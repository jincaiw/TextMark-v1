...
export function ExportDialog({ open, locale, onExportHtml, onExportPng, onExportPdf, onClose }: ExportDialogProps) {
  ...
  const item = (icon: React.ReactNode, label: string, hint: string, action: () => void) => (
    <button className="export-option" onClick={() => { action(); onClose(); }}>
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={onClose} ref={backdropRef}>
    <section className="export-dialog" role="dialog" aria-modal="true" aria-labelledby="export-title" onMouseDown={(event) => event.stopPropagation()}>
      <header><h2 id="export-title">{t(locale, "export")}</h2><button onClick={onClose} aria-label={t(locale, "close")}><X /></button></header>
