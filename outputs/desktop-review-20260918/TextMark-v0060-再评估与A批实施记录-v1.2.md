...
- **参照版本**：Markdown Preview `v0.0.60`，tag/main `d4a033aaf0d31028175a4f6195602fe2a2ed5134`（本轮公开仓库 API 与固定提交源码复核）
...
| T07 | Find.swift:89–93：查找覆盖层不在 tabs 上方、不重排 tabs | FindBar 位于 Toolbar 与 tabs 之间 | 第二批移入文档工作区；与格式条叠放并正确预留编辑内容，不破坏滚动交接 | 待实施 |
| T08 | 原生文档标签+格式覆盖层 | 没有双标签截图却曾标完成 | 首批补双文档真实场景与断言；后续层级改造再回归 | 待验证 |
...
当前状态（2026-09-25）：Toolbar/Sidebar 第二批已完成代码、门禁和交互证据验证，已提交至 `main` 并创建 `v0.10.7` 标签发布；原生 NSToolbar/titlebar 像素级证据仍独立保留。
