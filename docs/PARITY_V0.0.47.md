...
| Menus | File/Edit/View/Format/Go/Window/Help with macOS-standard shortcuts (⌥⌘1-3, ⇧⌘M, ⌃⌘1-3, ⌘L, ⌘T/⌘W) | Complete | Rust `build_menu`, `menu-command` wiring, keyboard layer |
| Localization | English and Simplified Chinese | Complete | `i18n.test.ts`, component tests |
| Toolbar | Persistent add/remove/reorder, split Open With / Open in LLM, Export PDF item, Safari-style discrete zoom | Complete | `ToolbarCustomizer`, settings v4 migration |
...
| macOS Quick Look | shared offline renderer, appearance and relative assets | Release gate | Universal extension, Xcode tests and mounted-DMG registration smoke |
